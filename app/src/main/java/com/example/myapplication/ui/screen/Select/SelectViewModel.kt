            ...
            when(select){
                SelectKey.AllMaterials -> {
                    /*viewModelScope.launch(Dispatchers.IO) {
                        val materialCardList = repository.materials
                            .first()
                            .map { material ->
                                CardItem(
...
*/                      val materialCardList = materialsType.map{item -> CardItem(id = "0", name = item.name, description = "ciao", type = JobType.valueOf(item.type), checked = item.checked)}
                        _state.update {
                            it.copy(
